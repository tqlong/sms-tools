...
from dftModel import dftAnal, dftSynth  # noqa: E402
"""
...
if __name__ == "__main__":
    ...
    x = genSine(1, 40, 0, 10000, 1) + genSine(1, 100, 0, 10000, 1) + \
        genSine(1, 200, 0, 10000, 1) + genSine(1, 2000, 0, 10000, 1)
    y, yfilt = suppressFreqDFTmodel(x, 10000, 16384)

    print(minimizeEnergySpreadDFT(y, 10000, 100, 200))
    print(minimizeEnergySpreadDFT(yfilt, 10000, 100, 200))

    import matplotlib.pyplot as plt
    plt.figure(), plt.plot(x)
    plt.figure(), plt.plot(y)
    plt.figure(), plt.plot(yfilt)
    plt.show()
