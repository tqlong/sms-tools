...
def testRealEven(x):
    ...
    M = len(x)
    hM1 = M//2
    x = np.append(x[hM1:], x[:hM1])
    X = fft(x, n=M)
    imagX = np.imag(X)
    imagIsZero = np.all(np.abs(imagX) < 1e-6)
    if imagIsZero:
        realX = np.real(X[1:])
        isRealEven = True if np.all(
            np.abs(realX - realX[::-1]) < 1e-6) else False
    else:
        isRealEven = False
    return isRealEven, x, X


if __name__ == "__main__":
    print(testRealEven(np.array([1, 2, 3, 4, 1, 2, 3])))
